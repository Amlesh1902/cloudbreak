package com.sequenceiq.datalake.service.atlas.check_atlas_updated;
...
public class CheckAtlasUpdatedService {
    @Value("${sdx.stack:check_atlas_updated.sleeptime_sec:2}")
    private int checkAtlasUpdatedSleepTimeSec;

    @Value("${sdx.stack.check_atlas_updated.duration_min:10}")
    ...
    public void verifyAtlasUpToDate(SdxCluster sdxCluster) {
        ...
        cloudbreakPoller.pollFlowStateByFlowIdUntilComplete(
                "check atlas service up to date", flowId.getPollableId(), sdxCluster.getId(), pollingConfig
        );
    }
}
