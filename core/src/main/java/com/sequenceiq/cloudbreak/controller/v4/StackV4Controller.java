...
public class StackV4Controller extends NotificationController implements StackV4Endpoint {
    ...
    @Override
    @CheckPermissionByAccount(action = AuthorizationResourceAction.POWERUSER_ONLY)
    public UpgradeV4Response checkForClusterUpgradeByName(Long workspaceId, String name, @NotNull UpgradeV4Request request,
            @AccountId String accountId) {
    ...
    @Override
    @InternalOnly
    public FlowIdentifier checkAtlasUpdated(Long workspaceId, String crn, @InitiatorUserCrn String initiatorUserCrn) {
        return stackOperationService.checkAtlasUpdated(NameOrCrn.ofCrn(crn), restRequestThreadLocalService.getRequestedWorkspaceId());
    }
}
